import require$$0, { forwardRef, createElement, useRef, useState, useEffect, useCallback } from 'react';
...
function Avatar({ className, ...props }) {
    return (jsxRuntimeExports.jsx(AvatarPrimitive.Root, { "data-slot": "avatar", className: cn("relative flex size-8 shrink-0 overflow-hidden rounded-full", className), ...props }));
}
function AvatarFallback({ className, ...props }) {
    return (jsxRuntimeExports.jsx(AvatarPrimitive.Fallback, { "data-slot": "avatar-fallback", className: cn("bg-muted flex size-full items-center justify-center rounded-full", className), ...props }));
}
...
function ScrollArea({ className, children, ...props }) {
    return (jsxRuntimeExports.jsxs(ScrollAreaPrimitive.Root, { "data-slot": "scroll-area", className: cn("relative", className), ...props, children: [jsxRuntimeExports.jsx(ScrollAreaPrimitive.Viewport, { "data-slot": "scroll-area-viewport", className: "focus-visible:ring-ring/50 size-full rounded-[inherit] transition-[color,box-shadow] outline-none focus-visible:ring-[3px] focus-visible:outline-1", children: children }), jsxRuntimeExports.jsx(ScrollBar, {}), jsxRuntimeExports.jsx(ScrollAreaPrimitive.Corner, {})] }));
}
function ScrollBar({ className, orientation = "vertical", ...props }) {
    return (jsxRuntimeExports.jsx(ScrollAreaPrimitive.ScrollAreaScrollbar, { "data-slot": "scroll-area-scrollbar", orientation: orientation, className: cn("flex touch-none p-px transition-colors select-none", orientation === "vertical" &&
            "h-full w-2.5 border-l border-l-transparent", orientation === "horizontal" &&
            "h-2.5 flex-col border-t border-t-transparent", className), ...props, children: jsxRuntimeExports.jsx(ScrollAreaPrimitive.ScrollAreaThumb, { "data-slot": "scroll-area-thumb", className: "bg-border relative flex-1 rounded-full" }) }));
}
...
const AreaMensagens = forwardRef(({ mensagens, estaDigitando, refFinal }, ref) => {
...
const ChatWidget$1 = ({ initialMessages = [], placeholder = "Digite sua mensagem...", onSendMessage, position = "left", primaryColor = "hsl(var(--primary))", title = "Assistente Virtual", welcomeMessage = "Olá! Como posso ajudá-lo hoje?", onToggle, defaultOpen = false, height = "max", className, isTyping: externalTypingState, onTypingChange, }) => {
    ...
    const [mensagens, setMensagens] = useState(initialMessages.length > 0
        ? initialMessages
        : [
            {
                id: "1",
    ...
    // Referências
    const finalMensagensRef = useRef(null);
    const inputRef = useRef(null);
...
function useIndicadorDigitacao({ duracao = 1500 } = {}) {
     ...
     */
    const digitarPorTempo = useCallback((tempoMs = duracao) => {
...
export { AreaEntrada, AreaMensagens, BotaoToggle, CabecalhoChat, ChatWidget, IndicadorDigitacao, MensagemChat, useChatbot, useIndicadorDigitacao };
